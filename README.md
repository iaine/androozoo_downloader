...
```
download.py            # core library + CLI
test_download.py       # pytest suite
pyproject.toml         # packaging, Python floor, extras, entry point
azkey.toml             # optional config (template; fill in or override via flags)
ui/
  app.py               # pywebview bridge (reuses download.py)
  assets/
    index.html
    styles.css
    app.js
...
This also installs an `azdownload` console command (equivalent to running
`python download.py`).
...
# or, equivalently:
python download.py --key YOUR_KEY --input hashes.txt --out ./apks
```
...
If an `azkey.toml` with real (non-placeholder) values sits next to
`download.py`, the key and output folder are pre-filled.
...
- **Cancel:** start a larger batch and cancel mid-run; the run stops and reports
  ...
  limitations.)*
- **Prefill:** with a real-valued `azkey.toml` beside `download.py`, confirm the
...
`window.evaluate_js(...)` (progress events) in `ui/app.py`.
