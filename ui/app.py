...
Thin pywebview shell around download.py. The Python side exposes a small JS API
(parse/validate hashes, start a download, cancel, pick an output folder) and
pushes progress events back into the page via evaluate_js. All heavy lifting is
delegated to download.py so the CLI and UI share one implementation.
...
# Make download.py importable whether it sits at the repo root or in downloader/.
_ROOT = Path(__file__).resolve().parent.parent
for candidate in (_ROOT, _ROOT / "downloader"):
    if (candidate / "download.py").exists():
        sys.path.insert(0, str(candidate))
        break

import download as az  # noqa: E402  (path set up above)
...
class Api:
    ...
    def _validate(hashes_text: str) -> tuple[list[str], int]:
        ...
        for raw in (hashes_text or "").splitlines():
            ...
            if not sha or sha.startswith("#"):
                continue
            if len(sha) == az.SHA256_LENGTH and all(c in "0123456789abcdefABCDEF" for c in sha):
    ...
    def prefill(self) -> dict:
        """Load key/output dir from azkey.toml next to download.py, if present."""
        for base in (_ROOT, _ROOT / "downloader"):
            cfg_path = base / "azkey.toml"
            try:
                cfg = az._read_config(cfg_path)
            except Exception:
                cfg = {}
            if cfg:
                key = cfg.get("key", "")
    ...
    def preview(self, hashes_text: str) -> dict:
        valid, invalid = self._validate(hashes_text)
        return {"valid": len(valid), "invalid": invalid}
